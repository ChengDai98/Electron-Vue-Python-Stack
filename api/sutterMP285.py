...
class sutterMP285 :
	...
	def __init__(self):
		...
		try:
			...
			#self.ser = serial.Serial(port='COM4',baudrate=9600,bytesize=serial.EIGHTBITS,parity=serial.PARITY_NONE,stopbits=serial.STOPBITS_ONE,timeout=self.timeOut)
			self.connected = 1
			if self.verbose:
				print(self.ser)
	...
	def getPosition(self):
		...
		xyz_um = numpy.array(struct.unpack('lll', xyzb[:12]))/self.stepMult
	...
	def setVelocity(self,Vel,vScalF=10):
		...
		# abb not sure if BOTH lines are in if???
		if vScalF == 50:
			velb2 = numpy.double(struct.unpack('B',velb[1])) + 128
	...
	def getStatus(self):
		...
		self.stepMult = numpy.double(statusbytes[25])*256 + numpy.double(statusbytes[24])
		...
		self.currentVelocity = numpy.double(127 & statusbytes[29])*256+numpy.double(statusbytes[28])
